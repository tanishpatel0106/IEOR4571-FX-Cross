...
from utils.lstm import LSTMModule
...
X_train_lstm = X_train.values.reshape(len(X_train), 1, X_train.shape[1])
X_test_lstm = X_test.values.reshape(len(X_test), 1, X_test.shape[1])
X_full_lstm = X.values.reshape(len(X), 1, X.shape[1])
...
lstm.fit(X_train_lstm, y_train.values)

# ---- Predictions ----
proba_train = lstm.predict_proba(X_train_lstm)
proba_test = lstm.predict_proba(X_test_lstm)

y_pred_train = (proba_train > 0.5).astype(int)
y_pred_test = (proba_test > 0.5).astype(int)

# ---- Attach probability to full index ----
proba_full = lstm.predict_proba(X_full_lstm)
...
fa = lstm.permutation_importance_lstm(X_test_lstm, y_test.values, feature_cols)
...
with st.expander("🔬 SHAP Explainability (Plotly Version)", expanded=False):
    ...
    X_sample = X.sample(min(300, len(X)), random_state=42)
    shap_wrapper = lstm.get_shap_predict_proba_wrapper()
    explainer = shap.KernelExplainer(shap_wrapper, X_sample)
    shap_values = explainer(X_sample)

...
def load_fx(symbol, timeframe, start_date, end_date):
    ...
    if isinstance(df.columns, pd.MultiIndex):
        ...
        if symbol in tickers:
            ...
            # fallback: drop first level (common when only one ticker returned)
            df = df.droplevel(0, axis=1)
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)

    # normalize casing and fill Close from Adj Close if needed
    ...
    if not all(c in df for c in required):
        if "Close" in df:
            ...
            st.error("Downloaded data missing OHLC fields; please refresh or change symbol/timeframe.")
            st.stop()
...
def build_feature_matrix(df_ohlc, alpha, beta, delta_pt):
    ...
    # handle adj close-only frames
    if "Close" not in df_ohlc and "Adj Close" in df_ohlc:
        df_ohlc["Close"] = df_ohlc["Adj Close"]
    ...
    <div style="padding: 1rem 1.25rem; border-radius: 0.9rem;
      ...
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
            ...
            Train an XGBoost classifier to predict hitting a same-bar profit target (Δ), then overlay it on the α–β strategy as an ML filter.
            </p>
        </div>
...
p_up = pd.Series(proba_full, index=X.index, name="P(hit)")
...
st.markdown(
    ...
    <div class="section-header">
      ...
      <p>XGBoost classifier predicting same-bar profit target hit (High ≥ (1+Δ)·Open).</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <div class="section-header">
      ...
      <p>Feature importances from XGBoost; optional SHAP values if installed.</p>
    </div>
    """,
    unsafe_allow_html=True,
)

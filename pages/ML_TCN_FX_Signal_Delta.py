...
# ========================================================
# LIGHTWEIGHT TCN IMPLEMENTATION (no external dependency)
# ========================================================
class SimpleTCN(layers.Layer):
    """
    Minimal Temporal Convolutional Network block:
    - Stacks dilated causal Conv1D residual blocks
    - Returns the last time step representation if return_sequences=False
    """
    def __init__(
        self,
        nb_filters=32,
    ...
    def build(self, input_shape):
        ...
        self.match_conv = layers.Conv1D(self.nb_filters, 1, padding="same")
    ...
    def call(self, inputs, training=None):
        ...
        for block in self.res_blocks:
            ...
            if x.shape[-1] != self.nb_filters:
                x = self.match_conv(x)
            x = layers.Add()([x, y])
        if self.return_sequences:
            return x
        # return last timestep representation
        return x[:, -1, :]
...
def build_trade_log(prices, raw_signals):
    """Create a simple trade log based on signal flips."""
    prices = prices.astype(float)
    raw_signals = raw_signals.reindex(prices.index)
...
def backtest(prices, raw_signals, positions, cost=0.0001, starting_capital=10000):
    ...
    trades_df = build_trade_log(prices, raw_signals)

    return strat_returns_pct, equity, metrics, trades_df, drawdown


def delta_backtest(df_ohlc, raw_signals, cost=0.0001, starting_capital=10000, delta_pt=0.005):
    """Enter at Open when signal==1; exit same bar at target or Close."""
    df = df_ohlc[["Open", "High", "Close"]].astype(float)
    sig = raw_signals.reindex(df.index).fillna(0).astype(float)
    ...
    for i, s in enumerate(sig):
        if s == 1:
            ...
            target = (1 + delta_pt) * o
            hit = h >= target
            exit_price = target if hit else c
            pnl_pct = delta_pt if hit else (c / o - 1)
            rets.append(pnl_pct)
            trades.append({
    ...
    strat_returns_pct = pd.Series(rets, index=df.index)
    strat_returns_pct -= turnover * cost
    ...
    metrics = {
        "Starting Capital": float(starting_capital),
        "Final Value": float(equity.iloc[-1]),
        "Net Profit": float(equity.iloc[-1] - starting_capital),
        "Total Return %": float((equity.iloc[-1] / starting_capital - 1) * 100 if starting_capital else 0.0),
        "Annualized Return": float(strat_returns_pct.mean() * 252),
        "Volatility": float(strat_returns_pct.std() * np.sqrt(252)),
        "Sharpe Ratio": float((strat_returns_pct.mean() * 252) / (strat_returns_pct.std() * np.sqrt(252)) if strat_returns_pct.std() > 0 else 0.0),
        "Max Drawdown": float(drawdown.max() if len(drawdown) > 0 else 0.0),
        "Number of Trades (signals)": int((sig == 1).sum()),
    }
...
def build_feature_matrix(df_ohlc, alpha, beta, delta_pt):
    """
    Build ML features + target:
    - Features: returns, volatility, MA diff, ES diff, RSI, etc.
    - Target: same-bar profit target hit (1 = High >= (1+Δ)*Open)
    """
    needed_cols = ["Open", "High", "Low", "Close"]
    cols_lower = {c.lower(): c for c in df_ohlc.columns}
    ...
    if not all(col in df_ohlc.columns for col in needed_cols):
        if "Close" in df_ohlc.columns:
            close_only = df_ohlc["Close"]
            ...
            st.error("Price data missing OHLC fields; please refresh or change symbol.")
            st.stop()

    df = pd.DataFrame(index=df_ohlc.index)

    open_px = df_ohlc["Open"]
    close = df_ohlc["Close"]
    high  = df_ohlc["High"]
    low   = df_ohlc["Low"]
    ...
    # Part 2 label: profit target hit within bar
    df["y"] = (high >= (1 + delta_pt) * open_px).astype(int)
    ...
    feature_cols = [col for col in df.columns if col not in ["price", "y"]]
...
def make_tcn_sequences(X, y, window=30):
    X_seq, y_seq = [], []

    for i in range(window, len(X)):
        X_seq.append(X.iloc[i-window:i].values)
...
with st.sidebar:
    ...
    delta_pt = st.number_input(
        "Δ profit target (e.g., 0.005 = 0.5%)",
        min_value=0.0,
        max_value=0.2,
        value=0.005,
        step=0.001,
    ...
    ml_long_cutoff = st.slider(
        "Min P(High ≥ (1+Δ)·Open) to allow LONG", 0.50, 0.90, 0.55, 0.01
    )
    ml_short_cutoff = st.slider(
        "Min P(down) (deprecated for Δ target)", 0.50, 0.90, 0.55, 0.01
    )
    st.caption("Short cutoff ignored for Δ-based targets; longs only.")
      ...
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
          ...
          <p style="color:#9CA3AF; font-size:0.85rem; margin-top:0.15rem;">
            ...
            Train a Temporal Convolutional Network to predict hitting a same-bar profit target (Δ), then overlay it on the α–β strategy as an ML filter.
          </p>
        </div>
...
feat_df, X, y, feature_cols = build_feature_matrix(df_px, alpha, beta, delta_pt)

n_samples = len(X)
train_size = int(n_samples * train_frac)
WINDOW = 30

X_seq, y_seq = make_tcn_sequences(X, y, window=WINDOW)

seq_train_size = int(len(X_seq) * train_frac)
...
# ---- Predictions ----
proba_train = model.predict(X_train).flatten()
proba_test  = model.predict(X_test).flatten()

y_pred_train = (proba_train > 0.5).astype(int)
y_pred_test  = (proba_test > 0.5).astype(int)

X_full_seq, _ = make_tcn_sequences(X, y, window=WINDOW)
proba_full = model.predict(X_full_seq).flatten()
...
st.markdown(
    ...
    <div class="section-header">
      ...
      <p>TCN classifier predicting same-bar profit target hit (High ≥ (1+Δ)·Open).</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <div class="section-header">
      ...
      <p>ML model filters the α–β signals: only take longs when P(hit Δ target) meets threshold; shorts ignored.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
# ML-filtered signals
raw_hybrid = pd.Series(0, index=raw_es.index)
raw_hybrid[(raw_es == 1) & (p_up_aligned >= ml_long_cutoff)] = 1
pos_hybrid = raw_hybrid.replace(0, np.nan).ffill().fillna(0)

# Backtest both
strat_es, equity_es, metrics_es, trades_es, dd_es = backtest(
    prices, raw_es, pos_es, cost=cost, starting_capital=starting_capital
)
strat_hybrid, equity_hybrid, metrics_hybrid, trades_hybrid, dd_hybrid = delta_backtest(
    df_px, raw_hybrid, cost=cost, starting_capital=starting_capital, delta_pt=delta_pt
)
...
p_last_down = 1.0 - p_last_up  # unused but retained for display compatibility
...
with col_h:
    ...
    st.markdown(
        f"<div class='kpi-sub'>P(hit Δ target): {p_last_up:.3f}<br>Price: {last_price:.5f}<br>Bar: {last_ts}</div>",
        unsafe_allow_html=True,
    )
    if flip_hybrid:
...
tab_price, tab_equity, tab_proba = st.tabs(
    ["📈 Price & Signals", "📉 Equity Curves", "📊 P(hit) Path"]
)
...
with tab_proba:
    proba_fig = px.line(
        p_up_aligned,
        labels={"index": "Date", "value": "P(hit Δ)"},
    )
...
# ========================================================
# 8.5 TRADE LOGS – ES VS HYBRID
# ========================================================
st.markdown(
    """
    <div class="section-header">
      <h3>Trade Log</h3>
      <p>Completed trades based on signal flips for the pure ES strategy and the ML-filtered hybrid.</p>
    </div>
    """,
    unsafe_allow_html=True,
)

tab_tr_es, tab_tr_h = st.tabs(["📜 ES Trades", "🤖 Hybrid Trades"])

with tab_tr_es:
    if trades_es.empty:
        st.info("No completed trades for the current configuration.")
    else:
        st.dataframe(
            trades_es.sort_values("Entry Time"),
            use_container_width=True,
            height=350,
        )
        st.caption(f"Total trades: {len(trades_es)}")

with tab_tr_h:
    if trades_hybrid.empty:
        st.info("No completed hybrid trades for the current configuration.")
    else:
        st.dataframe(
            trades_hybrid.sort_values("Entry Time"),
            use_container_width=True,
            height=350,
        )
        st.caption(f"Total trades: {len(trades_hybrid)}")
...
#     """
#     <div class="section-header">
#       <h3>Explainability – What Drives P(hit Δ)?</h3>
